...
export function RoughRect({
  ...
  const { animateOnHover: contextHover } = useCrumble();
  const { drawRect, svgRef, theme } = useRough({
    stableId: id,
    theme: themeProp,
    variant: "border",
  });
  const shouldAnimateHover = animateOnHoverProp ?? contextHover;
  ...
  const draw = useCallback(
    (reseed = false) => {
      ...
      const node = drawRect(inset, inset, width - inset * 2, height - inset * 2, {
        fill: fill ?? "none",
        roughness: rounded ? 0.5 : undefined,
    ...
    [drawRect, fill, rounded, stroke, svgRef],
  );
